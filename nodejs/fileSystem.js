//读取文件    异步fs.readFile()和同步fs.readFileSync() 
...
//stats.isFile()是文件返回true
//stats.isDirectory()是目录返回true
//stats.isBlockDevice()是块设备返回true
//stats.isCharacterDevice()是字符设备返回true
//stats.isSymbolicLink()是软链接返回true
//stats.isFIFO()是FIFO返回true,FIFO是UNIX中的一种特殊类型的命令管道
//stats.isSocket()如果是 Socket 返回true
...
//写入文件fs.writeFile(path,data[,options],callback)
...
//callback - 回调函数,有三个参数err,bytesRead, buffer,err 为错误信息,bytesRead表示读取的字节数,buffer为缓冲区对象
...
//删除文件fs.unlunk(path,callback);


//var fs=require("fs");
//fs.unlink("file.txt",function(err,data){
//	if(err){
...
//创建目录fsmkdir(path[,mode],callback);
//mode - 设置目录权限,默认为 0777


//var fs=require("fs");
//fs.mkdir("/nodejs/node",function(err){//同一文件不能重复创建,否则报错
//	if(err){
//		return console.log(err);
//	}
//	console.log("目录创建成功")
...
//var fs=require("fs");
//fs.rmdir("/nodejs/node1",function(err){//文件夹不存在时报错
//	if(err){
//		return console.log(err);
//	}
//	console.log("删除文件夹成功");
...
//16	fs.lchmod(path, mode, callback)异步 lchmod().回调函数没有参数,但可能抛出异常Only available on Mac OS X.
...
//63	fs.watch(filename[, options][, listener])查看 filename 的修改,filename 可以是文件或目录返回 fs.FSWatcher 对象
...
//fs.writeFile("file.txt","这是一段测试文字",function(err){
//	if(err){
//		return console.log(err);
//	}
//	console.log("保存成功");
...
fs.appendFile('file.txt','这是添加的测试数据',function(err){
	if(err){
		return console.log(err);
	}
	console.log("数据添加成功");
